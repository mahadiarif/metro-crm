...
@section('breadcrumb')
<a href="{{ route('tyro-dashboard.reports.sales') }}">Reports</a>
<span>/ Sales</span>
@endsection
...
<div style="display: flex; gap: 1rem; margin-bottom: 1.5rem; flex-wrap: wrap;">
    <div class="card" style="flex: 1; min-width: 160px; padding: 1.25rem 1.5rem;">
        ...
        <div style="font-size: 1.75rem; font-weight: 700; margin-top: 0.25rem;">{{ number_format($aggregates->total_deals) }}</div>
    </div>
    <div class="card" style="flex: 1; min-width: 160px; padding: 1.25rem 1.5rem;">
        <div style="font-size: 0.8125rem; color: #10b981; font-weight: 500;">Total Revenue</div>
        <div style="font-size: 1.75rem; font-weight: 700; margin-top: 0.25rem; color: #10b981;">৳ {{ number_format($aggregates->total_revenue, 2) }}</div>
    </div>
    <div class="card" style="flex: 1; min-width: 160px; padding: 1.25rem 1.5rem;">
        ...
        <div style="font-size: 1.75rem; font-weight: 700; margin-top: 0.25rem;">
            ৳ {{ $aggregates->total_deals > 0 ? number_format($aggregates->total_revenue / $aggregates->total_deals, 2) : '0.00' }}
        </div>
    </div>
</div>
...
<div class="card" style="margin-bottom: 1.5rem;">
    ...
    <div class="card-body">
        ...
        <div style="display: flex; gap: 0.5rem; margin-bottom: 1.25rem; flex-wrap: wrap;">
            ...
            @endforeach
            @if(request()->anyFilled(['quick','start_date','end_date','user_id','service_id','lead_status','zone']))
                <a href="{{ route('tyro-dashboard.reports.sales') }}" class="btn btn-ghost" style="font-size: 0.8125rem; padding: 0.3rem 0.75rem; height: auto; color: var(--muted-foreground);">✕ Clear all</a>
            @endif
        </div>

        {{-- Advanced Filters --}}
        <form action="{{ route('tyro-dashboard.reports.sales') }}" method="GET">
            <div class="form-row" style="flex-wrap: wrap; gap: 1rem; align-items: flex-end;">
                ...
                <div class="form-group" style="margin-bottom: 0; flex: 1; min-width: 140px;">
                    ...
                    <select name="lead_status" class="form-select">
                        ...
                        @foreach(['new', 'contacted', 'interested', 'closed', 'lost'] as $st)
                            <option value="{{ $st }}" {{ request('lead_status') == $st ? 'selected' : '' }}>{{ ucfirst($st) }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group" style="margin-bottom: 0; flex: 1; min-width: 140px;">
...
<div class="card">
    <div class="card-body" style="padding: 0;">
            ...
            <table class="table">
                ...
                <tbody>
                    @forelse($sales as $i => $sale)
                        <tr>
                            <td style="color: var(--muted-foreground); font-size: 0.8125rem;">{{ ($sales->currentPage() - 1) * $sales->perPage() + $i + 1 }}</td>
                            <td style="white-space: nowrap;">{{ $sale->closed_at->format('d M Y') }}</td>
                            <td>
                                <div style="font-weight: 600;">{{ $sale->lead->company_name ?? 'N/A' }}</div>
                                <div style="font-size: 0.75rem; color: var(--muted-foreground);">{{ $sale->lead->client_name ?? '' }}</div>
                            </td>
                            <td>{{ $sale->user->name ?? 'N/A' }}</td>
                            <td><span class="badge badge-secondary">{{ $sale->service->name ?? 'N/A' }}</span></td>
                            <td style="text-align: right; font-weight: 700; color: #10b981; white-space: nowrap;">৳ {{ number_format($sale->amount, 2) }}</td>
                            <td><span style="font-size: 0.8125rem; color: var(--muted-foreground);">{{ Str::limit($sale->remarks, 60) }}</span></td>
                        </tr>
                    @empty
                        <tr>
        ...
        <div style="padding: 1.25rem 1.5rem; border-top: 1px solid var(--border);">
            {{ $sales->links('tyro-dashboard::partials.pagination') }}
        </div>
        @endif
    </div>
</div>
