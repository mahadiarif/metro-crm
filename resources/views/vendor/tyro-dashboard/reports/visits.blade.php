...
@section('breadcrumb')
<a href="{{ route('tyro-dashboard.reports.visits') }}">Reports</a>
<span>/ Visits</span>
@endsection
...
<div class="page-header">
    <div class="page-header-row">
        <div>
            ...
            <p class="page-description">Analyze client visits and field executive interactions.</p>
        </div>
...
<div style="display: flex; gap: 1rem; margin-bottom: 1.5rem; flex-wrap: wrap;">
    <div class="card" style="flex: 1; min-width: 160px; padding: 1.25rem 1.5rem;">
        <div style="font-size: 0.8125rem; color: var(--muted-foreground); font-weight: 500;">Total Visits</div>
        <div style="font-size: 1.75rem; font-weight: 700; margin-top: 0.25rem;">{{ $visits->total() }}</div>
    </div>
</div>
...
<div class="card" style="margin-bottom: 1.5rem;">
    ...
    <div class="card-body">
        ...
        <div style="display: flex; gap: 0.5rem; margin-bottom: 1.25rem; flex-wrap: wrap;">
            ...
            @endforeach
            @if(request()->anyFilled(['quick','start_date','end_date','user_id','lead_id','service_id','lead_status','zone']))
                <a href="{{ route('tyro-dashboard.reports.visits') }}" class="btn btn-ghost" style="font-size: 0.8125rem; padding: 0.3rem 0.75rem; height: auto; color: var(--muted-foreground);">✕ Clear all</a>
            @endif
        </div>

        <form action="{{ route('tyro-dashboard.reports.visits') }}" method="GET">
            <div class="form-row" style="flex-wrap: wrap; gap: 1rem; align-items: flex-end;">
                ...
                <div class="form-group" style="margin-bottom: 0; flex: 1; min-width: 160px;">
                    ...
                    <select name="lead_id" class="form-select">
                        ...
                        @foreach($leads as $lead)
                            <option value="{{ $lead->id }}" {{ request('lead_id') == $lead->id ? 'selected' : '' }}>{{ $lead->company_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group" style="margin-bottom: 0; flex: 1; min-width: 160px;">
                ...
                <div class="form-group" style="margin-bottom: 0; flex: 1; min-width: 140px;">
                    ...
                    <select name="lead_status" class="form-select">
                        ...
                        <option value="interested" {{ request('lead_status') == 'interested' ? 'selected' : '' }}>Interested</option>
...
<div class="card">
    <div class="card-body" style="padding: 0;">
            ...
            <table class="table">
                ...
                <tbody>
                    @forelse($visits as $i => $visit)
                        <tr>
                            <td style="color: var(--muted-foreground); font-size: 0.8125rem;">{{ ($visits->currentPage() - 1) * $visits->perPage() + $i + 1 }}</td>
                            <td style="white-space: nowrap;">{{ $visit->visit_date->format('d M Y') }}</td>
                            <td>
                                <div style="font-weight: 600;">{{ $visit->lead->company_name ?? 'N/A' }}</div>
                                <div style="font-size: 0.75rem; color: var(--muted-foreground);">{{ $visit->lead->client_name ?? '' }}</div>
                            </td>
                            <td>{{ $visit->user->name ?? 'N/A' }}</td>
                            <td style="text-align: center;"><span class="badge badge-secondary">Visit {{ $visit->visit_number }}</span></td>
                            <td>{{ $visit->service->name ?? '—' }}</td>
                            <td><span style="font-size: 0.8125rem; color: var(--muted-foreground);">{{ Str::limit($visit->meeting_notes, 50) }}</span></td>
                            <td style="font-size: 0.8125rem;">{{ $visit->location ?? '—' }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" style="text-align: center; padding: 4rem; color: var(--muted-foreground);">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="width: 40px; height: 40px; margin: 0 auto 1rem; display: block; opacity: 0.4;"><path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" /></svg>
        ...
        <div style="padding: 1.25rem 1.5rem; border-top: 1px solid var(--border);">
            {{ $visits->links('tyro-dashboard::partials.pagination') }}
        </div>
        @endif
    </div>
</div>
